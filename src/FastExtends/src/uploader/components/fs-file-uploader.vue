  ...
  export default {
    ...
    setup(props, ctx) {
      ...
      const computedOnLimit = (isCheck = false) => {
        ...
        return (
          props.limit > 0 && fileListLocal.value.length >= props.limit + add
        )
      }

      function showLimitTip() {
        ui.message.instance.warning(
          t('fs.extends.fileUploader.limitTip', [props.limit]),
        )
      }
      function checkLimit() {
        ...
        if (props.sizeLimit != null) {
          const limit =
            typeof props.sizeLimit === 'number'
              ? props.sizeLimit
              : props.sizeLimit.limit
          const limitTip =
            typeof props.sizeLimit === 'number'
              ? `文件大小不能超过${computeFileSize(limit)}`
              : props.sizeLimit.tip
          const showMessage = () => {
            ui.message.instance.warning(limitTip)
          }
      ...
      const beforeUpload = async (file, list) => {
        ...
        if (props.beforeUpload) {
          const ret = await props.beforeUpload({
            file,
            fileList: fileListLocal.value,
          })
          if (ret === false) {
            return
          }
      ...
      function isPicture() {
        return (
          props.listType === ui.upload.typeImageCard ||
          props.listType === ui.upload.typeImage
        )
      }
